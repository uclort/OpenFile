...
extension NSButton {
    ...
    func addTarget(sendAction: NSEvent.EventTypeMask = [.leftMouseUp, .leftMouseDown, .rightMouseUp, .rightMouseDown], action:@escaping (NSButton) -> Void ) {
        let eventObj = AssociatedClosureClass(eventClosure: action)
        eventClosureObj = eventObj
        target = self
        self.action = #selector(eventExcuate(_:))
        self.sendAction(on: sendAction)
    }
